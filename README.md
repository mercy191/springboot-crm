...
4. **Сборка и запуск:** \
    Для сборки исполняемого файла воспользуйтесь командой:
   ```bash
   ./gradlew clean bootJar
   ```
   Для запуска исполняемого файла воспользуйтесь командой:
    ```bash
   java -jar build/libs/springboot-crm-0.0.1-SNAPSHOT.jar
   ```
